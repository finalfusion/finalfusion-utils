...
impl FinalfusionApp for ReconstructApp {
    fn app() -> App<'static, 'static> {
        App::new("reconstruct")
            .about("Reconstruct embedding matrix from quantized embeddings")
            .arg(
                Arg::with_name(INPUT)
    ...
    fn run(&self) {
        ...
        let len = embeddings.vocab().vocab_len();
        ...
        let nd_norms = match norms {
            None => NdNorms::new(Array1::ones(len)),
            Some(inner) => inner,
        };
...
fn reconstruct_storage(storage: &StorageWrap) -> NdArray {
    ...
    reconstructions
        ...
        .into_par_iter()
        .enumerate()
        .for_each(|(idx, mut reconstruction)| reconstruction.assign(&storage.embedding(idx)));
